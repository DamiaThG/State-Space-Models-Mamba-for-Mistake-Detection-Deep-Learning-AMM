...
* **RecentPastBlock:** Focalizza l'analisi su finestre temporali recenti molto strette (es. $[30, 90, 150]$ frame), estraendo 5 snippet locali fissi per catturare transizioni visive ad alta frequenza e micromovimenti dell'operatore, considerando che in media una *fine-grained action* dura 1.7 secondi.
...
2. **Mamba Backbone:** Il nucleo poggia su una pila di blocchi Mamba causali. Per questa implementazione, il modello istanzia nativamente la libreria ufficiale in C++ e CUDA (`mamba_ssm`) per garantire la massima accelerazione hardware e l'ottimizzazione del footprint in VRAM. I layer sono protetti da connessioni residuali stabili per scongiurare il vanishing gradient nella backpropagation.
3. **Classification Head:** Una rete MLP che riduce lo spazio $d_{\text{model}}$ nei logit tridimensionali necessari per la classificazione a tre classi mutuamente esclusive.
...
* **Focal Loss (con Class Weighting):** Affrontare lo sbilanciamento estremo e la rarità intrinseca delle classi `mistake` e `correction` ha richiesto l'abbandono della tradizionale Cross-Entropy in favore della **Focal Loss**. Questa funzione di costo, definita come $FL(p_t) = -\alpha_t (1 - p_t)^\gamma \log(p_t)$, mitiga dinamicamente l'impatto degli esempi facili e preponderanti (i frame `correct` in cui il modello è molto confidente), costringendo i gradienti a concentrarsi sulle transizioni rare e difficili da classificare. A questa modulazione (regolata dal parametro $\gamma$) è stato affiancato un moltiplicatore di peso per classe $\alpha_t$ calcolato in modo inversamente proporzionale alla frequenza delle classi (ed eventualmente elevato a un iperparametro `class_weight_exp`), penalizzando così doppiamente gli errori sulle classi minoritarie.
...
* In direzione opposta, l'estrema efficienza dell'architettura si conferma nella configurazione minimale a soli 3.19M parametri, che difende una Test Loss solida di 0.5340, dimostrando come l'efficienza predittiva dell'SSM poggi sull'ingegneria del routing dell'attenzione temporale piuttosto che sulla mera forza bruta parametrica.
...
### 6.5 Limitazioni Intrinseche del Setting *Single-View*
Un ulteriore fattore di divergenza rispetto al *benchmark* originale risiede nella dimensionalità del dominio spaziale prospettico. L'architettura proposta dagli autori di Assembly101 opera in un regime *multi-view* massivo, aggregando feature estratte simultaneamente da 12 angolazioni differenti (8 videocamere statiche e 4 prospettive egocentriche).
Al fine di soddisfare i vincoli computazionali e le specifiche del presente progetto, l'analisi è stata deliberatamente confinata a un setting *single-view*, vincolando il modello all'osservazione esclusiva della prospettiva empiricamente più informativa. Tale riduzione drastica del campo visivo introduce inevitabili occlusioni e limita l'intelligibilità dell'azione negli spazi tridimensionali complessi, costituendo un collo di bottiglia strutturale che ridimensiona fisiologicamente l'orizzonte prestazionale massimo rispetto ai risultati *multi-camera* documentati in letteratura.

---

## 7. Analisi Qualitativa e Failure Cases

Dall'ispezione empirica e dall'analisi della matrice di predizione dei modelli scaturisce che l'accuratezza sulla classe di background `correct` rimane quasi stazionaria e invulnerabile (F1 costantemente superiore al 93-95%). Le inefficienze sui target anomali si aggregano intorno a due pattern prevalenti di *Failure Cases*:
...
2. **Integrazione di Pipeline Multimodali:** Arginare le ambiguità percettive puramente visive (che intrappolano perfino gli oracoli) fondendo in ingresso flussi secondari espliciti, come array descrittivi delle posizioni e del tracking delle mani nello spazio 3D, o flussi testuali generativi per ancorare le transizioni d'azione ad una logica simbolica.